...
namespace HappyBrowser.Controls.BrowserTabStrip {
	public class BrowserTabStripItemCollection : CollectionWithEvents {
		...
		public virtual BrowserTabStripItem? GetFirstVisible(string workgroup)
		{
            for (int i = 0; i < base.Count; i++)
            {
		...
		protected override void OnInsertComplete(int index, object item) {
			BrowserTabStripItem fATabStripItem = (BrowserTabStripItem)item;
            if (!string.IsNullOrEmpty(fATabStripItem.Name) && fATabStripItem != null)
			{
